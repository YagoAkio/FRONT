...
const urlBase = 'https://back-delta-gray.vercel.app/fornecedor';
//Thunks
...
export const buscarFornecedores = createAsyncThunk('fornecedor/buscarFornecedores', async () => {
    try { 
        ...
        return {
            status: false,
            listaFornecedores: [],
            mensagem: 'Ocorreu um erro ao recuperar as fornecedores da base de dados:' + erro.message
        }
    }
});
...
export const adicionarFornecedor = createAsyncThunk('fornecedor/adicionar', async (fornecedor) => {
    const resposta = await fetch(urlBase, {
        ...
        return {
            status: false,
            mensagem: 'Ocorreu um erro ao adicionar a fornecedor:' + erro.message
        }
    });
    if (resposta.ok) {
        const dados = await resposta.json();
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(removerFornecedor.rejected, (state, action) => {
                state.mensagem = "Erro ao remover a fornecedor: " + action.error.message;
                state.estado = ESTADO.ERRO;
            })
    }
});
